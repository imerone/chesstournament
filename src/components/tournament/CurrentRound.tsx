...
const CurrentRound = ({ roundId, roundNumber }: CurrentRoundProps) => {
  ...
  // Pairings + board_results for current round — auto refresh
  const { data: pairings } = useQuery({
    ...
    queryFn: async () => {
      ...
      // attach board_results for each pairing
  ...
  if (!roundId) {
    return (
      <Card className="mb-8 shadow-elegant border-accent/20">
        ...
        <CardContent>
          <p className="text-muted-foreground">Текущий раунд ещё не создан</p>
          <Button asChild className="mt-3">
            <Link to="/rounds">Создать раунд</Link>
          </Button>
        </CardContent>
      </Card>
    );
    ...
    <Card className="mb-8 shadow-elegant border-accent/20">
      ...
      <CardContent>
              ...
              {pairing.is_bye ? (
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-lg font-bold">{Number(pairing.team_a_points).toFixed(1)}</span>
                  </div>
                  <div className="h-px bg-border" />
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-sm">{pairing.team_b?.short_code}</span>
                    <span className="text-lg font-bold">{Number(pairing.team_b_points).toFixed(1)}</span>
                  </div>
        ...
        <div className="mt-2 text-xs text-muted-foreground">
          Автообновление каждые 5 сек (пауза, если вкладка скрыта).
        </div>
      </CardContent>
    </Card>
  );
